...
abstract class ThinkOauth{
	 ...
	 * 请求code 
	 */
	public function getRequestCodeURL($from=null){
		$this->config();
		//Oauth 标准参数
		$params = array(
		...
		if($from == 'cycs_wd'){
			$params['redirect_uri'] = 'http://www.'.DOMAIN_URL.'/login/callback?type=sina';
		}else if($from == 'fyws_wd'){
			$params['redirect_uri'] = 'http://www.'.DOMAIN_URL.'/fyws/callback?type=sina';
		}
